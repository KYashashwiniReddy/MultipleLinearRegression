...
ax.plot(df['total_bill'],model.predict(scalar.transform(x)),color='pink')
...
st.markdown(f"""
    ...
    <p>Tip depends upon <b>Bill amount and size </b></p>
    </div>
""",unsafe_allow_html=True)
#---------------------------------------------#
# predictions #
...
size=st.slider("size",int(df['size'].min()),int(df['size'].max()),2)
input = scalar.transform([[bill,size]])
